...
private IList<IWebElement> _daysOnCal() {
        ...
        [FindsBy(How = How.XPath, Using = ".//*[@id='divGroupMemebersGrid']/tbody/tr/td[1]/label")]
        ...
        private void _to_userCheckAndSelect(IWebDriver driver,string to_user)
        {
            ...
            if (GetText(driver, _userFromUserGroupfield).Contains(to_user))
            {
                Click(driver, _userFromUserGroupfield);
            }
        }
        ...
        public bool OpenMailSpecial(IWebDriver driver, string strData, string encryptPass = "", bool withSubject = true, string text="")
        {
            bool flag = false;
            if (withSubject == false)
            {
        ...
        public bool SelectExternalDeptTo(string deptName = "", string deptCode = "", string type = "")
        {
            Thread.Sleep(4000);
            //Click(_driver, _externalDeptToBtn);
            //((IJavaScriptExecutor)_driver).ExecuteScript("arguments[0].click();", _externalDeptToBtn);
            try
            {
                try
                {
            ...
            catch
            {
                Console.WriteLine();
                Console.WriteLine();
                Console.WriteLine("-------------------------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Error! while clicking on External Department To Button");
                Console.WriteLine();
                Console.WriteLine();
            }
